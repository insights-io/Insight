...
public class ElasticsearchTestContainer extends ElasticsearchContainer {
  ...
  public void cleanup() throws IOException {
    ...
    client.deleteByQuery(new DeleteByQueryRequest("_all").setQuery(new MatchAllQueryBuilder()),
        RequestOptions.DEFAULT);

    client.indices().delete(new DeleteIndexRequest("_all"), RequestOptions.DEFAULT);

    await().atMost(10, TimeUnit.SECONDS).until(() -> {
      long documentCount = client.search(new SearchRequest("_all"), RequestOptions.DEFAULT)
          .getHits()
          .getTotalHits().value;
