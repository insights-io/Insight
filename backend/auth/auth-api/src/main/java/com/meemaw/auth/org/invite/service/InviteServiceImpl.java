...
public class InviteServiceImpl implements InviteService {
  ...
  public CompletionStage<InviteDTO> create(
      InviteCreateDTO inviteCreate, InsightPrincipal principal) {

    InviteCreateIdentifiedDTO teamInvite =
        new InviteCreateIdentifiedDTO(
            inviteCreate.getEmail(),
            principal.getOrg(),
            inviteCreate.getRole(),
            principal.getUserId());

    return pgPool.begin().thenCompose(transaction -> createTransactional(transaction, teamInvite));
  }

  private CompletionStage<InviteDTO> createTransactional(
      Transaction transaction, InviteCreateIdentifiedDTO teamInviteCreate) {
    String email = teamInviteCreate.getEmail();
    String org = teamInviteCreate.getOrg();
    log.info(
        "Inviting user {} to org {} with role {} by {}",
        email,
        org,
    ...
    return inviteDatasource
        .create(transaction, teamInviteCreate)
        .thenCompose(
            teamInvite ->
                sendInviteEmail(teamInvite.getToken(), teamInvite)
                    .exceptionally(
                        throwable -> {
  ...
  private CompletionStage<Boolean> acceptTransactional(
      Transaction transaction, InviteAcceptDTO inviteAccept) {
    ...
    return inviteDatasource
        .findTransactional(transaction, email, org, token)
        .thenApply(
            maybeTeamInvite -> {
              InviteDTO teamInvite =
                  maybeTeamInvite.orElseThrow(
                      () -> {
                        log.info(
                            "Invite does not exist user={} org={} token={}", email, org, token);
