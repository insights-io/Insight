...
public class SignUpServiceImpl implements SignUpService {
  ...
  public CompletionStage<Optional<UUID>> signUp(
      URI serverBaseUri, SignUpRequestDTO signUpRequestDTO) {
    ...
    SignUpRequest signUpRequest =
        SignUpRequest.builder()
            ...
            .redirect(signUpRequestDTO.getRedirect())
            .build();

    return sqlPool
        .beginTransaction()
        .thenCompose(transaction -> signUp(serverBaseUri, signUpRequest, transaction));
  }

  private CompletionStage<Optional<UUID>> signUp(
      URI serverBaseUri, SignUpRequest signUpRequest, SqlTransaction transaction) {
        ...
        .thenCompose(
            emailTaken -> {
              ...
              return signUpDatasource
                  ...
                  .thenCompose(
                      token ->
                          sendSignUpCompleteEmail(email, serverBaseUri, token)
                              .exceptionally(
                                  throwable -> {
                                    transaction.rollback();
                                    log.error(
  ...
  private CompletionStage<Void> sendSignUpCompleteEmail(
      String email, URI serverBaseUri, UUID token) {
    String subject = "Finish your registration";
    String completeSignUpLocation =
        UriBuilder.fromUri(serverBaseUri)
    ...
    return completeSignUpTemplate
        ...
        .data("name", SharedConstants.REBROWSE_ORGANIZATION_NAME)
        .renderAsync()
        .thenCompose(
            html ->
                mailer
  ...
  public CompletionStage<AuthUser> ssoSignUp(String email, String fullName) {
  ...
  public CompletionStage<AuthUser> ssoSignUpNewMember(
